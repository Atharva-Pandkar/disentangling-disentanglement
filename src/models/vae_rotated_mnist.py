...
class RandomRotationWithRecording(object):
    def __init__(self):
    ...
    def get_recorded_angles(self):
        return self.recorded_angles
class Enc(nn.Module):
    ...
    def forward(self, x):
        pre_shape = x.size()[:-3]
        #print(f' pre shape \t {pre_shape}')
        #print(x.view(-1, *torch.Size([1, 32, 32])).shape)
...
class RotatedMNIST(VAE):
    ...
    def getDataLoaders(batch_size, shuffle=True, device="cuda"):
        ...
        custom_transform = RandomRotationWithRecording()

        # Compose the custom transform with other required transforms
        transform = transforms.Compose([
            custom_transform,
            transforms.Resize(size=(32, 32)),
            transforms.ToTensor(),  # Add other transforms if needed
            transforms.Lambda(lambda p: p.clamp(Constants.eta, 1 - Constants.eta))
        ])

        train_loader = DataLoader(
            datasets.MNIST('../data/rotated_mnist', train=True, download=True, transform=transform),
            batch_size=batch_size, shuffle=shuffle, **kwargs)
        test_loader = DataLoader(
            datasets.MNIST('../data/fashion_mnist', train=False, download=True, transform=transform),
            batch_size=batch_size, shuffle=shuffle, **kwargs)

        # Access the recorded angles if needed
        recorded_angles = custom_transform.get_recorded_angles()
